...
class CobottaGripper:
    ...
    def move_force(self, force: int, direction: bool) -> Tuple[bool, int]:
        ...
        param = [clip_force, direction, "Next"]
    ...
    def move(self, position: float, speed: int, force: int) -> Tuple[bool, int]:
        ...
        if abs(self._current_position - clip_pos) > self._pos_diff_threthold:
            if clip_pos <= self._min_position:
                self.move_force(clip_force, True)
                print("froce grip!")
            else:
                self.move_position(clip_pos, clip_speed)
                print("pos grip!")
